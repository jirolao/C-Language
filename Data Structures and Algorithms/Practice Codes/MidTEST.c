...
void initVHeap(VHeap *V) {
	int x;
	for(x = 0;x < VH_SIZE - 1;x++){
		V->VH_node[x].next = x + 1;
		strcpy(V->VH_node[x].elem.prodID, "EMPTY");
		strcpy(V->VH_node[x].elem.prodDesc.name, "EMPTY");
	}
	V->avail = 0;
	strcpy(V->VH_node[x].elem.prodID, "EMPTY");
	strcpy(V->VH_node[x].elem.prodDesc.name, "EMPTY");
...
void displaySet(cursorSet A) {
	...
	for(x = A.elemIndex;x != -1;x = A.VHptr->VH_node[x].next){
		printf("\n%-7s%-12s%-15s%-10d", A.VHptr->VH_node[x].elem.prodID, A.VHptr->VH_node[x].elem.prodDesc.name, A.VHptr->VH_node[x].elem.prodDesc.weight, x);
	}
...
openDic initOpenDict(VHeap *VH) {
	...
	for(i = 0;i < OPEN_DSIZE;i++){
		Dictionary.header[i] = -1;
	}
	return Dictionary;
}

openDic convertToOpenDict(cursorSet *A) {
	openDic newDic = initOpenDict(A->VHptr);
    int x;

    // Iterate through the cursor set and insert elements into the open hash dictionary
    for (x = A->elemIndex; x != -1; x = A->VHptr->VH_node[x].next) {
        // Calculate the hash value for the current element's ID
        int transfer = openHash(A->VHptr->VH_node[x].elem.prodID);
        // Pointer to traverse the linked list at the hash index
        int *trav = &newDic.header[transfer];
        
        // Traverse the linked list to find the correct position for insertion
        while (*trav != -1 && strcmp(A->VHptr->VH_node[*trav].elem.prodID, A->VHptr->VH_node[x].elem.prodID) < 0) {
            trav = &newDic.dicVHptr->VH_node[*trav].next;
        }
        
        // Insert the element into the open hash dictionary if it's not already present
        if (*trav == -1 || strcmp(A->VHptr->VH_node[*trav].elem.prodID, A->VHptr->VH_node[x].elem.prodID) != 0) {
    ...
    // Reset the cursor set after conversion
    A->elemIndex = -1;
...
void displayOpenDict(openDic D)
{
	...
	for(x = 0;x < OPEN_DSIZE;x++){
		for(y = D.header[x]; y != -1;y = D.dicVHptr->VH_node[y].next){
			printf("%-7sd%-15s", x, D.dicVHptr->VH_node[y].elem.prodID);
		}
	}
