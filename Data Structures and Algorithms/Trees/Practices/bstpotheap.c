...
//---Priority Queue---
void enqueuePriorityQueue(minHeap *H, product P);
product dequeuePriorityQueue(minHeap *H);
...
void enqueuePriorityQueue(minHeap *H, product P) {
    ...
    H->prod[H->lastNdx] = P;
    int i = H->lastNdx;
    while (i > 0 && H->prod[i].prodPrice < H->prod[(i - 1) / 2].prodPrice) {
        swapProduct(&H->prod[i], &H->prod[(i - 1) / 2]);
        i = (i - 1) / 2;
    }
}

product dequeuePriorityQueue(minHeap *H) {
    product temp = H->prod[0];
    H->prod[0] = H->prod[H->lastNdx];
    H->lastNdx--;
    heapify(H);
    return temp;
}
